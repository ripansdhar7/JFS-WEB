...
<head>
  ...
  <meta name="keywords" content="bootstrap, bootstrap4" />
		...
		<title>Admin Login | Order manager</title>
		<style>
			html{
				font-size: 100%;
			}
...
if(isset($_POST['login'])){
   ...
   $select_users = mysqli_query($conn, "SELECT * FROM `admin_orders` WHERE username = '$uname' AND password = '$pass'") or die('query failed');
   ...
   if(mysqli_num_rows($select_users) > 0){
      ...
      while ($row = mysqli_fetch_assoc($select_users)) {
         ...
         $_SESSION['admin_order_id'] = $row['id'];
         header('location:orders-area.php');
